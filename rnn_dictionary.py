...
class Dictionary:
    ...
    def __init__(self, trainingData: list[list[str]]) -> None:
        ...
        self.word2index = {list_of_unique_words[i]: i + 3 for i in range(len(list_of_unique_words))}
...
def remove_special_symbols(ls: list[list[str]]) -> list[list[str]]:
    for sentence in ls:
        sentence[:] = [word for word in sentence if
                       word != config.unknown and word != config.sentence_start and word != config.sentence_end and word != config.padding]
    return ls
